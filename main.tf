...
resource "aci_rest_managed" "spanRsDestToVPort" {
  for_each = { for destination in var.destinations : destination.name => destination if destination.tenant != null && destination.application_profile != null && destination.endpoint_group != null && destination.client_endpoint != null }
  ...
  dn = "${aci_rest_managed.spanVDest[each.value.name].dn}/rsdestToVPort-[${format("uni/tn-%s/ap-%s/epg-%s/cep-%s", each.value.tenant, each.value.application_profile, each.value.endpoint_group, each.value.client_endpoint)}]"
